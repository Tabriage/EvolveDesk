    ...
    <main className={`app-shell ${activeView === "topics" ? "topic-mode" : activeView === "graph" ? "graph-mode" : ""}`}>
      ...
      {activeView === "lab" ? (
        ...
        <aside className="agent-panel vault-guide-panel">
          ...
          <ol className="vault-guide-steps"><li><i>1</i><div><strong>设备私钥</strong><p>ECDH 与 ECDSA 私钥不可导出，只保存在本机 IndexedDB。</p></div></li><li><i>2</i><div><strong>明确授权</strong><p>创建设备核对指纹后，才向目标设备包装同步密钥。</p></div></li><li><i>3</i><div><strong>可验恢复</strong><p>预签恢复授权与旧同步包双重验证，接管后强制轮换新密钥。</p></div></li><li><i>4</i><div><strong>字段预检</strong><p>有共同父版本时按字段合并，冲突必须明确选择后才能写入。</p></div></li></ol>
