export const MAX_EXTERNAL_MEDIA_BYTES = 500 * 1024 * 1024;

const CHUNK_BYTES = 4 * 1024 * 1024;
const BLOCK_BYTES = 64;
const INITIAL_STATE = new Uint32Array([
  0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
  0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19,
]);
const ROUND_CONSTANTS = new Uint32Array([
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
]);

function rotateRight(value, bits) {
  return (value >>> bits) | (value << (32 - bits));
}

class StreamingSha256 {
  constructor() {
    this.state = INITIAL_STATE.slice();
    this.buffer = new Uint8Array(BLOCK_BYTES);
    this.bufferLength = 0;
    this.totalBytes = 0;
    this.finished = false;
    this.words = new Uint32Array(64);
  }

  processBlock(bytes, offset = 0) {
    const words = this.words;
    for (let index = 0; index < 16; index += 1) {
      const cursor = offset + index * 4;
      words[index] = ((bytes[cursor] << 24) | (bytes[cursor + 1] << 16) | (bytes[cursor + 2] << 8) | bytes[cursor + 3]) >>> 0;
    }
    for (let index = 16; index < 64; index += 1) {
      const before15 = words[index - 15];
      const before2 = words[index - 2];
      const sigma0 = rotateRight(before15, 7) ^ rotateRight(before15, 18) ^ (before15 >>> 3);
      const sigma1 = rotateRight(before2, 17) ^ rotateRight(before2, 19) ^ (before2 >>> 10);
      words[index] = (words[index - 16] + sigma0 + words[index - 7] + sigma1) >>> 0;
    }
    let [a, b, c, d, e, f, g, h] = this.state;
    for (let index = 0; index < 64; index += 1) {
      const sum1 = rotateRight(e, 6) ^ rotateRight(e, 11) ^ rotateRight(e, 25);
      const choose = (e & f) ^ (~e & g);
      const temp1 = (h + sum1 + choose + ROUND_CONSTANTS[index] + words[index]) >>> 0;
      const sum0 = rotateRight(a, 2) ^ rotateRight(a, 13) ^ rotateRight(a, 22);
      const majority = (a & b) ^ (a & c) ^ (b & c);
      const temp2 = (sum0 + majority) >>> 0;
      h = g;
      g = f;
      f = e;
      e = (d + temp1) >>> 0;
      d = c;
      c = b;
      b = a;
      a = (temp1 + temp2) >>> 0;
    }
    this.state[0] = (this.state[0] + a) >>> 0;
    this.state[1] = (this.state[1] + b) >>> 0;
    this.state[2] = (this.state[2] + c) >>> 0;
    this.state[3] = (this.state[3] + d) >>> 0;
    this.state[4] = (this.state[4] + e) >>> 0;
    this.state[5] = (this.state[5] + f) >>> 0;
    this.state[6] = (this.state[6] + g) >>> 0;
    this.state[7] = (this.state[7] + h) >>> 0;
  }

  update(value) {
    if (this.finished) throw new Error("完整媒体哈希已经结束");
    const bytes = value instanceof Uint8Array ? value : new Uint8Array(value);
    this.totalBytes += bytes.byteLength;
    let offset = 0;
    if (this.bufferLength) {
      const available = Math.min(BLOCK_BYTES - this.bufferLength, bytes.byteLength);
      this.buffer.set(bytes.subarray(0, available), this.bufferLength);
      this.bufferLength += available;
      offset += available;
      if (this.bufferLength === BLOCK_BYTES) {
        this.processBlock(this.buffer);
        this.bufferLength = 0;
      }
    }
    while (offset + BLOCK_BYTES <= bytes.byteLength) {
      this.processBlock(bytes, offset);
      offset += BLOCK_BYTES;
    }
    if (offset < bytes.byteLength) {
      this.buffer.set(bytes.subarray(offset), 0);
      this.bufferLength = bytes.byteLength - offset;
    }
  }

  digestHex() {
    if (this.finished) throw new Error("完整媒体哈希已经结束");
    this.finished = true;
    const finalLength = this.bufferLength < 56 ? BLOCK_BYTES : BLOCK_BYTES * 2;
    const final = new Uint8Array(finalLength);
    final.set(this.buffer.subarray(0, this.bufferLength));
    final[this.bufferLength] = 0x80;
    const bitLength = this.totalBytes * 8;
    const high = Math.floor(bitLength / 0x100000000);
    const low = bitLength >>> 0;
    const view = new DataView(final.buffer);
    view.setUint32(finalLength - 8, high, false);
    view.setUint32(finalLength - 4, low, false);
    for (let offset = 0; offset < finalLength; offset += BLOCK_BYTES) this.processBlock(final, offset);
    return [...this.state].map((part) => part.toString(16).padStart(8, "0")).join("");
  }
}

function normalizeHashableFile(file) {
  if (!file || typeof file.slice !== "function") throw new Error("没有取得可读取的本地媒体文件");
  const size = Number(file.size);
  if (!Number.isSafeInteger(size) || size < 1) throw new Error("本地媒体文件大小无效");
  if (size > MAX_EXTERNAL_MEDIA_BYTES) throw new Error("外部原文件超过 500MB 安全上限");
  return size;
}

export async function createExternalMediaFullHash(file, onProgress) {
  const size = normalizeHashableFile(file);
  const hash = new StreamingSha256();
  let processed = 0;
  for (let offset = 0; offset < size; offset += CHUNK_BYTES) {
    const chunk = new Uint8Array(await file.slice(offset, Math.min(size, offset + CHUNK_BYTES)).arrayBuffer());
    hash.update(chunk);
    processed += chunk.byteLength;
    if (typeof onProgress === "function") onProgress(processed, size);
  }
  return hash.digestHex();
}
