...
async function main() {
  ...
  const branch = git("branch", "--show-current")
    ...
    || String(process.env.WORKERS_CI_BRANCH || "").trim();
  ...
  if (process.env.WORKERS_CI_COMMIT_SHA && process.env.WORKERS_CI_COMMIT_SHA.toLowerCase() !== commit) throw new Error("Workers Builds 提交与当前检出提交不一致");
