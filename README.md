...
任务、收件箱、习惯、个人路线、个人业务台及记录、创作定位、观察信号、选题、内容复盘、视频总结、知识卡片、复习卡与复习记录、已保存问答、周回顾和活动记录保存在浏览器 `localStorage`；体积更大的字幕单独保存在浏览器 `IndexedDB`。当前不依赖账户或云端数据库。即使不连接模型，也能配置路线与业务台、手动制卡、完成间隔复习、记录创作信号和发布结果，并完整使用日常工作台、字幕搜索与周度事实轨迹。
