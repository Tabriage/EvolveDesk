...
- **数据迁移**：把 `localStorage` 工作台状态与 `IndexedDB` 字幕、采样帧封装成一份带 SHA-256 完整性校验的 JSON 迁移卷。导入时先校验格式、版本和文件指纹，再按稳定对象 ID 预览新增、改写与移除；二次确认后才整体替换，并可在刷新页面前一步撤回整次恢复。
...
- 迁移卷只在浏览器中生成和读取，不调用模型或上传服务端；Base URL、模型名称、API 密钥和本地音视频原文件不会进入迁移卷。SHA-256 用于发现损坏或被改动的文件，不用于证明文件来源可信。
...
视频字幕、关键帧、OCR、视觉问答、多来源学习专题、修订审阅、证据星图、可逆本地数据迁移和提案分支审阅已经可用，后续继续补齐：
