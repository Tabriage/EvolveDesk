...
- **加密同步与冲突合并**：每台设备在独立 `IndexedDB` 中生成不可导出的 ECDH 密钥协商私钥与 ECDSA 签名私钥。目标设备先生成 24 小时授权请求；同步空间的创建设备读取请求、当面核对完整指纹并明确确认后，才生成只能由目标私钥解锁的授权回执。工作台仍复用迁移卷作为内层载荷，再封成带父版本、AES-256-GCM 认证加密和设备签名的同步包。文件适配器可以经 U 盘或用户选择的目录搬运密文；HTTP 条件对象适配器可由用户显式连接一个支持 CORS、强 ETag 与条件请求的 HTTPS 对象地址，只用 GET/PUT 搬运同一种不透明密文。浏览器最多保留每个空间最近 4 份加密同步包作为版本证据。导入分叉包且能找到共同父版本时，系统按稳定对象 ID 做三方预览：单边变化自动合入，双边修改同一对象才由用户逐项选择本机或迁入版本；整体迁入仍是单独的危险选择。写入前需要二次确认，本次可撤回；下一份同步包会认证两条已合并父版本线。
...
任务、收件箱、习惯、个人路线、个人业务台及记录、创作定位、观察信号、选题、内容复盘、视频总结、视觉证据元数据、知识卡片、学习专题、复习卡与复习记录、已保存问答、周回顾和活动记录保存在浏览器 `localStorage`；体积更大的字幕与关键帧图片单独保存在浏览器 `IndexedDB`；设备身份私钥与同步空间密钥保存在另一座独立 `IndexedDB` 密钥库。当前不依赖账户或云端数据库，远端对象地址与令牌也不会持久化。即使不连接模型或远端存储，也能配置路线与业务台、保存专题资料架、本机抽帧与 OCR、手动制卡、完成间隔复习、记录创作信号和发布结果，并完整使用日常工作台、字幕搜索、加密迁移、文件同步传输与周度事实轨迹。
...
- 同步 v1 是由创建设备逐台授权的星形信任模型：创建设备认识每个成员，成员只认识创建设备和自己。跨成员直接收包、真正的设备撤销与密钥轮换尚未实现；要撤销已经拿到空间密钥的设备，必须新建空间并重新授权仍可信的设备，界面不会把“从列表隐藏”伪装成撤销。
...
- 三方合并以工作台稳定对象 ID、创作定位、今日焦点、字幕来源和画面来源为选择单元。与共同父版本相比，只有一侧变化时自动采用变化侧；两侧结果不同才成为冲突，默认保留本机，用户可逐项切换为迁入对象，包括明确选择删除。最终结果再次经过完整状态迁移、引用约束和媒体来源清洗，避免选择后留下悬空任务、知识引用或孤立媒体。
- 确认三方合并后，迁入版本成为主父版本，本机原版本作为另一条待发布父线；下一次由用户生成同步包时，两条父线与合并密文一起接受加密和设备签名。双父版本使用同步包 v2，读取端仍兼容只有单父版本的 v1 包。撤回本次合并会同时恢复工作台、媒体库、原版本头与原合并父线。当前逐对象选择不等于字段级文本合并：同一个任务、路线、业务台或学习专题被两台设备分别改写时，需要选择其中一份完整对象。
...
视频字幕、关键帧、OCR、视觉问答、多来源学习专题、修订审阅、证据星图、口令加密、可逆本地数据迁移、版本兼容证明、设备授权、供应商无关的加密同步包、显式 HTTP 条件对象传输、逐对象三方合并与提案分支审阅已经可用，后续继续补齐：

1. 设备撤销、空间密钥轮换与剩余可信设备重新授权。
2. 大体积本地媒体原文件的可选外部存储索引。
3. 更细粒度的字段级冲突编辑与合并决策回执。
