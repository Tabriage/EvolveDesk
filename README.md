...
- **进化实验室**：在白名单源码内生成差异，逐文件审阅后只在临时 worktree 写入；ESLint 与 TypeScript 静态检查通过才创建独立提案分支和提交。用户再次确认后才能推送分支并开启 GitHub 草稿 PR，当前工作分支不会承载未采用的 Agent 代码。PR 合并后还需第三次确认：本地服务重新读取 GitHub 状态，仅在审阅头提交、基分支、合并提交、原始 Git 基线、变更文件闭集和最终文件哈希全部一致时，才用 `--ff-only` 同步当前分支；不自动解决冲突或夹带其他远端变化。
...
- 已合并提案不会自动进入工作区。第三次确认会重新查询 GitHub PR，要求合并头仍是通过验证的提交、远端 base tip 等于 GitHub 合并提交、当前分支仍在封存基线且工作区干净；远端相对基线只能改动提案文件，最终内容必须逐文件匹配封存 SHA-256。随后只允许 `git merge --ff-only`，任一漂移都停止并交回人工处理。
...
远端审阅需要本机已经登录 GitHub CLI（`gh auth status`）并配置 GitHub `origin`。点击“推送并开启草稿审阅”后还需再次确认；应用只推送当前提案分支，并以封存时的分支作为 PR base。审阅显示已合并后，“验证并同步已合并版本”还需再次点击确认；它不会切换分支、拉取其他范围变化、执行提案代码或自动创建合并提交。
