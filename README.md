...
视频字幕、关键帧、OCR、视觉问答、多来源学习专题、修订审阅、证据星图、口令加密、可逆本地数据迁移、版本兼容证明、设备授权、供应商无关的加密同步包、逐对象三方合并与提案分支审阅已经可用，后续继续补齐：

1. 用户显式连接的远端存储适配器与条件写入，不扩大当前加密协议的权限。
2. 设备撤销、空间密钥轮换与剩余可信设备重新授权。
3. 大体积本地媒体原文件的可选外部存储索引。
4. 更细粒度的字段级冲突编辑与合并决策回执。
