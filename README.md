...
- **行动 Agent**：读取有限的工作台快照，把自然语言目标转换为结构化动作清单，也能选择真实存在的路线动作、创建或推进业务记录并接入今日任务；只有用户确认后才写入本地状态。
- **自省记忆**：记录任务完成、输入整理和已经确认的 Agent 动作，解释工作台如何形成。
- **进化实验室**：在白名单源码内生成差异，逐文件审阅、固定验证并支持回滚。

任务、收件箱、习惯、个人路线、个人业务台及记录、视频总结、知识卡片、已保存问答、周回顾和活动记录保存在浏览器 `localStorage`；体积更大的字幕单独保存在浏览器 `IndexedDB`。当前不依赖账户或云端数据库。即使不连接模型，也能用模板或空白结构配置路线与业务台，并完整使用日常工作台、字幕搜索与周度事实轨迹。
...
- 行动 Agent 只能提出 `新增任务 / 设置焦点 / 保存输入 / 新增习惯 / 激活真实路线动作 / 新增业务记录 / 推进真实业务记录 / 从业务记录创建任务` 八类结构化动作；已有对象的 ID 必须来自当前有限快照。
