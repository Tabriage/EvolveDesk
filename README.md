...
- **加密同步传输**：每台设备在独立 `IndexedDB` 中生成不可导出的 ECDH 密钥协商私钥与 ECDSA 签名私钥。目标设备先生成 24 小时授权请求；同步空间的创建设备读取请求、当面核对完整指纹并明确确认后，才生成只能由目标私钥解锁的授权回执。工作台仍复用迁移卷作为内层载荷，再封成带父版本、AES-256-GCM 认证加密和设备签名的同步包。当前文件传输适配器可以经 U 盘、网盘目录或其他用户选择的路径搬运密文；导入后先验签、解密并显示顺序后继或版本分叉，再进入原有整体差异预检，绝不自动合并或覆盖。
...
- 同步包只判断首次版本、顺序后继、重复版本和分叉。分叉包可以解锁并进入整体替换预检，但不会自动合并；用户确认恢复后才推进本机版本头，本次撤回也会恢复原版本头。多设备逐对象冲突预览与可逆合并属于下一阶段。
...
视频字幕、关键帧、OCR、视觉问答、多来源学习专题、修订审阅、证据星图、口令加密、可逆本地数据迁移、设备授权、供应商无关的加密同步包与提案分支审阅已经可用，后续继续补齐：

1. 多设备版本冲突预览、逐对象选择与可逆合并模型。
2. 合并后的版本迁移提示与本地数据兼容检查。
3. 用户显式连接的远端存储适配器与条件写入，不扩大当前加密协议的权限。
