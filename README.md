...
任务、收件箱、习惯、个人路线、个人业务台及记录、创作定位、观察信号、选题、内容复盘、视频总结、知识卡片、已保存问答、周回顾和活动记录保存在浏览器 `localStorage`；体积更大的字幕单独保存在浏览器 `IndexedDB`。当前不依赖账户或云端数据库。即使不连接模型，也能配置路线与业务台、记录创作信号和发布结果，并完整使用日常工作台、字幕搜索与周度事实轨迹。
...
- 行动 Agent 只能提出 `新增任务 / 设置焦点 / 保存输入 / 新增习惯 / 激活真实路线动作 / 新增业务记录 / 推进真实业务记录 / 从业务记录创建任务 / 推进真实创作选题 / 从创作选题创建任务` 十类结构化动作；已有对象的 ID 必须来自当前有限快照。
