...
视频字幕、关键帧、OCR、视觉问答、外部原文件索引、多来源学习专题、修订审阅、证据星图、口令加密、可逆本地数据迁移、版本兼容证明、设备授权与密钥轮换、供应商无关的加密同步包、显式 HTTP 条件对象传输、逐对象三方合并与提案分支审阅已经可用，后续继续补齐：

1. 更细粒度的字段级冲突编辑与合并决策回执。
2. 创建设备遗失时的离线恢复材料与所有权迁移方案。
3. 外部媒体索引的失效清理、批量重新定位与完整哈希选项。
