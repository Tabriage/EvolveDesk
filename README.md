...
任务、收件箱、习惯、个人路线、个人业务台及记录、创作定位、观察信号、选题、内容复盘、视频总结、视觉证据元数据、知识卡片、学习专题、复习卡与复习记录、已保存问答、周回顾和活动记录保存在浏览器 `localStorage`；体积更大的字幕与关键帧图片单独保存在浏览器 `IndexedDB`；设备身份私钥与同步空间密钥保存在另一座独立 `IndexedDB` 密钥库。当前不依赖账户或云端数据库。即使不连接模型，也能配置路线与业务台、保存专题资料架、本机抽帧与 OCR、手动制卡、完成间隔复习、记录创作信号和发布结果，并完整使用日常工作台、字幕搜索、加密迁移、文件同步传输与周度事实轨迹。
...
- 同步层不会自动连接网盘、对象存储、账户或网络端点。当前文件适配器的下载与导入都需要用户手势；模型、行动 Agent 和源码 Agent 都不能创建空间、授权设备、生成授权回执、导出或导入同步包。
...
视频字幕、关键帧、OCR、视觉问答、多来源学习专题、修订审阅、证据星图、口令加密、可逆本地数据迁移、设备授权、供应商无关的加密同步包、逐对象三方合并与提案分支审阅已经可用，后续继续补齐：

1. 合并后的版本迁移提示与本地数据兼容检查。
2. 用户显式连接的远端存储适配器与条件写入，不扩大当前加密协议的权限。
3. 设备撤销、空间密钥轮换与剩余可信设备重新授权。
